...
export class CartService {
  ...
  async getAbandonedCarts(hours = 1) {
    const cutoffDate = new Date();
    cutoffDate.setHours(cutoffDate.getHours() - hours);

    // Encontrar carritos que no se han actualizado en X horas
    // Como los items están en cartItems, buscamos items antiguos agrupados por usuario
    ...
    for (const item of abandonedItems) {
      ...
      const cart = cartsByUser.get(item.userId);
      // Usar la fecha más reciente del carrito
      if (item.updatedAt > cart.lastUpdatedAt) {
        cart.lastUpdatedAt = item.updatedAt;
      }

      // Calcular precio
      let price = 0;
      if (item.productVariant?.priceUSD) price = Number(item.productVariant.priceUSD);
      else if (item.product.priceUSD) price = Number(item.product.priceUSD);
      
      const itemTotal = price * item.quantity;
      cart.totalPotentialUSD += itemTotal;

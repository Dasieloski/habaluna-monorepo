...
export function SmartImage({
  ...
  const imgSrc = useMemo(() => {
    const url = getImageUrl(src?.trim(), { width, height: typeof height === 'number' ? height : undefined })
    return url || null
  }, [src, width, height])
  ...
  // En reintentos, añadir parámetro para evitar usar respuesta en caché (404/fail)
  const srcWithRetry = retryCount > 0 ? `${imgSrc}${imgSrc.includes('?') ? '&' : '?'}_r=${retryCount}` : imgSrc

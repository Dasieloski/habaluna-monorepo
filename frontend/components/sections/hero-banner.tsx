          ...
          {displayBanners.map((banner, index) => (
            ...
            >
              <div className="relative h-full min-h-[450px] md:min-h-[550px]">
                ...
                {/* Overlay con gradiente sutil para mejorar legibilidad */}
                <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-black/10 to-transparent pointer-events-none" />
                
                <div className="absolute inset-0 flex flex-col justify-end p-6 md:p-12">
                  <div className="max-w-xl relative z-10">
                    {/* Fondo con blur detrás del texto (glassmorphism minimalista) */}
                    <div className="absolute -inset-4 md:-inset-6 bg-black/20 backdrop-blur-sm rounded-2xl -z-10" />
                    
                    <h1
                      className="font-heading text-3xl md:text-5xl lg:text-6xl font-bold text-white mb-2 md:mb-4 tracking-tight leading-tight drop-shadow-[0_2px_8px_rgba(0,0,0,0.5)]"
                    >
                      {banner.title}
                    </h1>
                    {banner.subtitle && (
                      <p className="text-base md:text-xl text-white/95 mb-4 md:mb-6 font-light drop-shadow-[0_2px_6px_rgba(0,0,0,0.4)]">
                        {banner.subtitle}
                      </p>
                    )}
                    {banner.buttonText && (
